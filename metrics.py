...
def apply_costs(fwd_ret: np.ndarray, cost_bps: float) -> np.ndarray:
    # Conservative: subtract round-trip cost on every signaled “trade”
...
def constrained_objective_foldscore(m: dict, min_signals: int, min_precision: float):
    # Hard penalties for constraint fails (prevents no-trade winners)
    ...
    # primary: net average return on signals
